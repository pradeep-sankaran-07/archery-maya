...
// Norwegian names for shop items — used by bilingual problem generation.
const ITEM_NAMES_NO = {
  chocolate: 'Sjokolade', apple: 'Eple', banana: 'Banan',
  toycar: 'Lekebil', stickers: 'Klistremerker', juice: 'Juiceboks',
  donut: 'Smultring', icecream: 'Is', pencil: 'Blyant',
  notebook: 'Notatbok', grapes: 'Druer', milk: 'Melk',
  cookies: 'Kjeks', strawberry: 'Jordbær', kite: 'Drage',
};
function iN(item, lang) { return lang === 'no' ? (ITEM_NAMES_NO[item.id] || item.name) : item.name; }
...
const TEMPLATES = [
  ...
  { id: 'kr_add2', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const [a, b] = pickN(rng, ITEMS, 2);
    const ans = a.price + b.price;
    return {
      prompt: lang === 'no'
      ? `Du kjøper en ${iN(a,lang)} ${a.emoji} og en ${iN(b,lang)} ${b.emoji}.\nHvor mye koster de til sammen?`
      : `You buy a ${iN(a,lang)} ${a.emoji} and a ${iN(b,lang)} ${b.emoji}.\nHow much do they cost together?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'kr_add3', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const [a, b, c] = pickN(rng, ITEMS, 3);
    const ans = a.price + b.price + c.price;
    return {
      prompt: lang === 'no'
      ? `Du kjøper en ${iN(a,lang)} ${a.emoji}, en ${iN(b,lang)} ${b.emoji} og en ${iN(c,lang)} ${c.emoji}.\nHva er totalen?`
      : `You buy a ${iN(a,lang)} ${a.emoji}, a ${iN(b,lang)} ${b.emoji}, and a ${iN(c,lang)} ${c.emoji}.\nWhat's the total?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'kr_add4', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const [a, b, c, d] = pickN(rng, ITEMS, 4);
    const ans = a.price + b.price + c.price + d.price;
    return {
      prompt: lang === 'no'
      ? `Handlevogn: ${iN(a,lang)} ${a.emoji}, ${iN(b,lang)} ${b.emoji}, ${iN(c,lang)} ${c.emoji}, ${iN(d,lang)} ${d.emoji}.\nTotalt?`
      : `Cart has: ${iN(a,lang)} ${a.emoji}, ${iN(b,lang)} ${b.emoji}, ${iN(c,lang)} ${c.emoji}, ${iN(d,lang)} ${d.emoji}.\nTotal?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'kr_change', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const item = pick(rng, ITEMS);
    const have = item.price + ri(rng, 3, 18);
    const ans = have - item.price;
    return {
      prompt: lang === 'no'
      ? `Du har ${have} kr.\nDu kjøper en ${iN(item,lang)} ${item.emoji} for ${item.price} kr.\nHvor mye er igjen?`
      : `You have ${have} kr.\nYou buy a ${iN(item,lang)} ${item.emoji} for ${item.price} kr.\nHow much is left?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'kr_two_step', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const [a, b] = pickN(rng, ITEMS, 2);
    ...
    return {
      prompt: lang === 'no'
      ? `Du kjøper en ${iN(a,lang)} ${a.emoji} (${a.price} kr) og en ${iN(b,lang)} ${b.emoji} (${b.price} kr).\nDu betaler med ${have} kr. Hvor mye tilbake?`
      : `You buy a ${iN(a,lang)} ${a.emoji} (${a.price} kr) and a ${iN(b,lang)} ${b.emoji} (${b.price} kr).\nYou pay with ${have} kr. How much change?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'kr_pay50', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const item = pick(rng, ITEMS);
    const have = 50;
    const ans = have - item.price;
    return {
      prompt: lang === 'no'
      ? `En ${iN(item,lang)} ${item.emoji} koster ${item.price} kr.\nDu betaler med en 50-kroneseddel.\nHvor mye får du tilbake?`
      : `A ${iN(item,lang)} ${item.emoji} costs ${item.price} kr.\nYou pay with a 50 kr note.\nHow much change do you get?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2, [0, 80])]),
      type: 'numeric',
    };
  } },
  { id: 'kr_afford', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const [a, b] = pickN(rng, ITEMS, 2);
    const total = a.price + b.price;
    const ans = (money ?? 10) >= total ? 'yes' : 'no';
    return {
      prompt: lang === 'no'
      ? `Du har ${money ?? 10} kr.\nDu vil ha en ${iN(a,lang)} ${a.emoji} (${a.price} kr) og en ${iN(b,lang)} ${b.emoji} (${b.price} kr).\nHar du råd til begge?`
      : `You have ${money ?? 10} kr.\nYou want a ${iN(a,lang)} ${a.emoji} (${a.price} kr) and a ${iN(b,lang)} ${b.emoji} (${b.price} kr).\nCan you afford both?`,
      correct: ans,
      choices: ['yes', 'no'],
      type: 'yesno',
    };
  } },
  { id: 'kr_split_change', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const [a, b, c] = pickN(rng, ITEMS, 3);
    ...
    return {
      prompt: lang === 'no'
      ? `Handlevogn: ${iN(a,lang)}, ${iN(b,lang)}, ${iN(c,lang)} (totalt ${total} kr).\nDu betaler ${have} kr. Tilbake?`
      : `Cart: ${iN(a,lang)}, ${iN(b,lang)}, ${iN(c,lang)} (total ${total} kr).\nYou pay ${have} kr. Change?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  ...
  { id: '2d_add_simple', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 12, 25);
    const b = ri(rng, 5, 14);
    const ans = a + b;
    return { prompt: `${a} + ${b} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: '2d_add_mid', difficulty: 3, generate(rng, money = 0, lang = 'en') {
  ...
  { id: '2d_sub_simple', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 14, 25);
    const b = ri(rng, 2, 9);
    const ans = a - b;
    return { prompt: `${a} − ${b} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: '2d_sub_mid', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 25, 50);
    const b = ri(rng, 8, 18);
    const ans = a - b;
    return { prompt: `${a} − ${b} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: '2d_sub_borrow', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    let a, b;
  ...
  { id: '2d_sub_big', difficulty: 4, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'add3_small', difficulty: 2, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'add3_mid', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const [a, b, c] = [ri(rng, 5, 14), ri(rng, 5, 14), ri(rng, 5, 14)];
    const ans = a + b + c;
    return { prompt: `${a} + ${b} + ${c} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'add4', difficulty: 3, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'add4_big', difficulty: 4, generate(rng, money = 0, lang = 'en') {
  ...
  // ─── Skip counting ─────────────────────────────────────────────────────────
  { id: 'skip2', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const start = ri(rng, 4, 14) * 2;
    const ans = start + 2;
    return {
      prompt: lang === 'no' ? `Tell med 2: ${start - 4}, ${start - 2}, ${start}, ?` : `Counting by 2s: ${start - 4}, ${start - 2}, ${start}, ?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'skip3', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const start = ri(rng, 2, 8) * 3;
    const ans = start + 3;
    return {
      prompt: lang === 'no' ? `Tell med 3: ${start - 6}, ${start - 3}, ${start}, ?` : `Counting by 3s: ${start - 6}, ${start - 3}, ${start}, ?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  ...
  { id: 'skip5', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    ...
    const ans = start + 5;
    return {
      prompt: lang === 'no' ? `Tell med 5: ${start - 10}, ${start - 5}, ${start}, ?` : `Counting by 5s: ${start - 10}, ${start - 5}, ${start}, ?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  ...
  { id: 'skip10', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    ...
    const ans = start + 10;
    return {
      prompt: lang === 'no' ? `Tell med 10: ${start - 20}, ${start - 10}, ${start}, ?` : `Counting by 10s: ${start - 20}, ${start - 10}, ${start}, ?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'skip_back2', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const start = ri(rng, 8, 18) * 2;
    const ans = start - 2;
    return {
      prompt: lang === 'no' ? `Tell tilbake med 2: ${start + 4}, ${start + 2}, ${start}, ?` : `Counting back by 2s: ${start + 4}, ${start + 2}, ${start}, ?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  ...
  { id: 'tens_in', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    ...
    return {
      prompt: lang === 'no' ? `Hvor mange tiere er det i ${n}?` : `How many tens are in ${n}?`,
      correct: tens,
  ...
  { id: 'ones_in', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    ...
    const n = tens * 10 + ones;
    return {
      prompt: lang === 'no' ? `Hvor mange enere er det i ${n}?` : `How many ones are in ${n}?`,
      correct: ones,
      choices: shuffle(rng, [ones, ...distinctDistractors(ones, 2, [0, 9])]),
      type: 'numeric',
    };
  } },
  { id: 'place_decompose', difficulty: 3, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'place_break', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const tens = ri(rng, 2, 7);
    const ones = ri(rng, 1, 9);
    const n = tens * 10 + ones;
    ...
    return {
      prompt: lang === 'no' ? `Hvilken viser ${n} delt i tiere + enere?` : `Which one shows ${n} broken into tens + ones?`,
      correct: right,
  ...
  { id: 'double_small', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const v = ri(rng, 3, 12);
    const ans = v + v;
    return {
      prompt: lang === 'no'
  ...
  { id: 'half_small', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const ans = ri(rng, 3, 12);
    const v = ans + ans;
    return {
      prompt: lang === 'no'
      ? `Halvparten av ${v}?\n(${v} delt i to like deler.)`
  ...
  { id: 'near_double', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const v = ri(rng, 4, 11);
    const ans = v + v + 1;
    return {
      prompt: lang === 'no'
      ? `${v} + ${v + 1} = ?\n(Tips: det er nesten dobbelt!)`
  ...
  { id: 'missing_add', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 2, 9);
  ...
  { id: 'missing_add_first', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const b = ri(rng, 2, 9);
    const ans = ri(rng, 2, 9);
    const total = b + ans;
    return {
  ...
  { id: 'missing_sub', difficulty: 3, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'missing_sub_minuend', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const b = ri(rng, 6, 14);
  ...
  // ─── Fact families ─────────────────────────────────────────────────────────
  { id: 'fact_family_sub', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 3, 8);
    const b = ri(rng, 3, 8);
    const total = a + b;
    return {
  ...
  { id: 'fact_family_add', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 3, 8);
    const b = ri(rng, 3, 8);
    const total = a + b;
    return {
  ...
  { id: 'compare_sums', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    ...
    return {
      prompt: lang === 'no'
      ? `${a1} + ${a2}    ?    ${b1} + ${b2}\nHvilket symbol passer i midten?`
  ...
  { id: 'compare_diffs', difficulty: 4, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'coins_5_1', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const fives = ri(rng, 1, 4);
    const ones = ri(rng, 1, 5);
    const ans = fives * 5 + ones;
    return {
      prompt: lang === 'no'
      ? `${fives} mynt${fives > 1 ? 'er' : ''} à 5 kr og ${ones} mynt${ones > 1 ? 'er' : ''} à 1 kr.\nHvor mye totalt?`
      : `${fives} coin${fives > 1 ? 's' : ''} worth 5 kr and ${ones} coin${ones > 1 ? 's' : ''} worth 1 kr.\nHow much in total?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'coins_10_1', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const tens = ri(rng, 1, 4);
    const ones = ri(rng, 1, 9);
    const ans = tens * 10 + ones;
    return {
      prompt: lang === 'no'
      ? `${tens} mynt${tens > 1 ? 'er' : ''} à 10 kr og ${ones} mynt${ones > 1 ? 'er' : ''} à 1 kr.\nHvor mye totalt?`
      : `${tens} coin${tens > 1 ? 's' : ''} worth 10 kr and ${ones} coin${ones > 1 ? 's' : ''} worth 1 kr.\nHow much in total?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'coins_10_5_1', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    ...
    return {
      prompt: lang === 'no'
      ? `${tens} × 10 kr + ${fives} × 5 kr + ${ones} × 1 kr.\nTotalt? (Legg dem bare sammen.)`
  ...
  { id: 'word_birds', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const start = ri(rng, 5, 12);
    const more = ri(rng, 2, 6);
    const ans = start + more;
    return {
      prompt: lang === 'no'
      ? `${start} fugler 🐦 sitter på en gren.\n${more} til flyr inn. Hvor mange fugler er det nå?`
      : `${start} birds 🐦 sit on a branch.\n${more} more fly in. How many birds now?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'word_cookies', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const start = ri(rng, 6, 14);
    const eat = ri(rng, 2, 5);
    const ans = start - eat;
    return {
      prompt: lang === 'no'
      ? `Tejas har ${start} kjeks 🍪.\nHan spiser ${eat}. Hvor mange kjeks er igjen?`
      : `Tejas has ${start} cookies 🍪.\nHe eats ${eat}. How many cookies are left?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'word_marbles', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const start = ri(rng, 8, 18);
    const lost = ri(rng, 2, 6);
    const ans = start - lost;
    return {
      prompt: lang === 'no'
      ? `Maya hadde ${start} kuler 🔵.\nHun mistet ${lost} under sofaen.\nHvor mange kuler har hun nå?`
      : `Maya had ${start} marbles 🔵.\nShe lost ${lost} under the sofa.\nHow many marbles does she have now?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'word_stickers', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const start = ri(rng, 4, 10);
    const gift = ri(rng, 3, 8);
    const ans = start + gift;
    return {
      prompt: lang === 'no'
      ? `Lily hadde ${start} klistremerker ✨.\nBestemor ga henne ${gift} til.\nHvor mange klistremerker har Lily nå?`
      : `Lily had ${start} stickers ✨.\nGrandma gave her ${gift} more.\nHow many stickers does Lily have now?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'word_apples', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const tree = ri(rng, 10, 20);
    const picked = ri(rng, 3, 8);
    const ans = tree - picked;
    return {
      prompt: lang === 'no'
      ? `Et epletre 🍎 hadde ${tree} epler.\n${picked} ble plukket.\nHvor mange er fortsatt på treet?`
      : `An apple tree 🍎 had ${tree} apples.\n${picked} were picked.\nHow many are still on the tree?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'word_pages', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const total = ri(rng, 20, 40);
    const read = ri(rng, 8, 16);
    const ans = total - read;
    return {
      prompt: lang === 'no'
      ? `Boken din har ${total} sider.\nDu har lest ${read}.\nHvor mange sider er igjen?`
      : `Your book has ${total} pages.\nYou've read ${read}.\nHow many pages are left?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'word_fish_tank', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 5, 12);
    const b = ri(rng, 4, 10);
    const ans = a + b;
    return {
      prompt: lang === 'no'
      ? `Et akvarium har ${a} gullfisk 🐟 og ${b} guppy-er.\nHvor mange fisk totalt?`
      : `A tank has ${a} goldfish 🐟 and ${b} guppies.\nHow many fish in all?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'word_balloons', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const start = ri(rng, 12, 22);
    const popped = ri(rng, 3, 7);
    const ans = start - popped;
    return {
      prompt: lang === 'no'
      ? `${start} ballonger 🎈 på festen.\n${popped} sprang!\nHvor mange ballonger er igjen?`
      : `${start} balloons 🎈 at the party.\n${popped} popped!\nHow many balloons are left?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  ...
  { id: 'twostep_add_add', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 3, 8);
    const b = ri(rng, 3, 8);
    const c = ri(rng, 2, 7);
    const ans = a + b + c;
    return {
      prompt: lang === 'no'
      ? `Maya har ${a} klistremerker.\nAlya gir henne ${b} til.\nSå gir Lily henne ${c} til.\nHvor mange klistremerker har Maya nå?`
  ...
  { id: 'twostep_add_sub', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    ...
    return {
      prompt: lang === 'no'
      ? `Du har ${start} kjeks 🍪.\nMamma baker ${more} til.\nSå spiser familien ${eat}.\nHvor mange kjeks er igjen?`
      : `You have ${start} cookies 🍪.\nMom bakes ${more} more.\nThen the family eats ${eat}.\nHow many cookies are left?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'twostep_kr', difficulty: 4, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'twostep_gift_spend', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    ...
    return {
      prompt: lang === 'no'
      ? `Du starter med ${start} kr.\nBestemor gir deg ${gift} kr.\nDu kjøper en ${iN(item,lang)} ${item.emoji} (${item.price} kr).\nHvor mye har du nå?`
      : `You start with ${start} kr.\nGrandma gives you ${gift} kr.\nYou buy a ${iN(item,lang)} ${item.emoji} (${item.price} kr).\nHow much do you have now?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  ...
  { id: 'count_on', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    ...
    return {
      prompt: lang === 'no'
      ? `Start på ${start}. Tell opp ${step}.\nHvor ender du?`
      : `Start at ${start}. Count up ${step}.\nWhere do you end?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  ...
  { id: 'count_back', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    ...
    return {
      prompt: lang === 'no'
      ? `Start på ${start}. Tell ned ${step}.\nHvor ender du?`
      : `Start at ${start}. Count back ${step}.\nWhere do you end?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'count_on_big', difficulty: 3, generate(rng, money = 0, lang = 'en') {
  ...
  // ─── +n patterns ───────────────────────────────────────────────────────────
  { id: 'pattern_plus3', difficulty: 3, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'pattern_plus4', difficulty: 3, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'pattern_plus5', difficulty: 2, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'pattern_minus', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const end = ri(rng, 1, 6);
    const ans = end + 2;
    return {
      prompt: lang === 'no'
      ? `Mønsteret går ned med 2.\nFyll inn: ${end + 8}, ${end + 6}, ${end + 4}, ?, ${end}`
      : `Pattern goes down by 2.\nFill in: ${end + 8}, ${end + 6}, ${end + 4}, ?, ${end}`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  ...
  // ─── Even / odd ────────────────────────────────────────────────────────────
  { id: 'pick_even', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const even = ri(rng, 1, 8) * 2;
    const odd1 = even + 1, odd2 = even - 3;
    return {
      prompt: lang === 'no' ? `Hvilken er et partall?` : `Which one is even?`,
      correct: String(even),
      choices: shuffle(rng, [String(even), String(odd1), String(odd2)]),
      type: 'choice',
    };
  } },
  { id: 'pick_odd', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const odd = ri(rng, 1, 8) * 2 + 1;
    const even1 = odd + 1, even2 = odd - 3;
    return {
      prompt: lang === 'no' ? `Hvilken er et oddetall?` : `Which one is odd?`,
      correct: String(odd),
  ...
  { id: 'time_min_extra', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const extra = ri(rng, 5, 30);
    return {
      prompt: lang === 'no'
      ? `1 time og ${extra} minutter er hvor mange minutter totalt?`
      : `1 hour and ${extra} minutes is how many minutes total?`,
      correct: 60 + extra,
      choices: shuffle(rng, [60 + extra, ...distinctDistractors(60 + extra, 2, [30, 150])]),
      type: 'numeric',
    };
  } },
  { id: 'time_days_week', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const extra = ri(rng, 1, 6);
    return {
      prompt: lang === 'no'
      ? `1 uke og ${extra} dager er hvor mange dager totalt?`
      : `1 week and ${extra} days is how many days total?`,
      correct: 7 + extra,
      choices: shuffle(rng, [7 + extra, ...distinctDistractors(7 + extra, 2, [5, 20])]),
      type: 'numeric',
    };
  } },
  { id: 'time_two_weeks', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    return {
      prompt: lang === 'no' ? `Hvor mange dager er det i 2 uker?` : `How many days are in 2 weeks?`,
      correct: 14,
  ...
  // ─── Make-10 strategy ──────────────────────────────────────────────────────
  { id: 'make10', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 1, 9);
    const ans = 10 - a;
    return {
      prompt: lang === 'no' ? `${a} + ? = 10\nHva passer i blanken?` : `${a} + ? = 10\nWhat goes in the blank?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2, [0, 10])]),
      type: 'numeric',
    };
  } },
  { id: 'make20', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 11, 19);
    const ans = 20 - a;
    return {
      prompt: lang === 'no' ? `${a} + ? = 20\nHva passer i blanken?` : `${a} + ? = 20\nWhat goes in the blank?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2, [0, 10])]),
      type: 'numeric',
    };
  } },
  { id: 'make100', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 5, 9) * 10;
    const ans = 100 - a;
    return {
      prompt: lang === 'no' ? `${a} + ? = 100\nHva passer i blanken?` : `${a} + ? = 100\nWhat goes in the blank?`,
      correct: ans,
  ...
  // ─── Bridging 10 / 20 ──────────────────────────────────────────────────────
  { id: 'bridge10', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 6, 9);
    const b = ri(rng, 4, 8);
  ...
  { id: 'bridge20', difficulty: 3, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'add_tens', difficulty: 2, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'sub_tens', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 5, 9) * 10;
  ...
  { id: 'add_round_then_some', difficulty: 3, generate(rng, money = 0, lang = 'en') {
  ...
  // ─── "Which is closest to" ─────────────────────────────────────────────────
  { id: 'closest_to', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const target = ri(rng, 15, 45);
    const correct = target;
    const off1 = target + ri(rng, 5, 10);
    const off2 = target - ri(rng, 5, 10);
    return {
      prompt: lang === 'no' ? `Hvilket er nærmest ${target}?` : `Which is closest to ${target}?`,
  ...
  { id: 'fives_make', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const ans = ri(rng, 2, 6);
    const target = ans * 5;
    return {
      prompt: lang === 'no' ? `Hvor mange 5 kr-mynter blir ${target} kr?` : `How many 5 kr coins make ${target} kr?`,
      correct: ans,
      choices: shuffle(rng, [ans, ans + 1, ans - 1]),
      type: 'numeric',
    };
  } },
  { id: 'tens_make', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const ans = ri(rng, 2, 6);
    const target = ans * 10;
    return {
      prompt: lang === 'no' ? `Hvor mange 10 kr-mynter blir ${target} kr?` : `How many 10 kr coins make ${target} kr?`,
      correct: ans,
      choices: shuffle(rng, [ans, ans + 1, ans - 1]),
      type: 'numeric',
    };
  ...
  // ─── Sub from 100 ──────────────────────────────────────────────────────────
  { id: 'sub_from_100', difficulty: 4, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'compare_num', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 15, 70);
    let b = ri(rng, 15, 70);
    while (a === b) b = ri(rng, 15, 70);
    const op = a > b ? '>' : '<';
    return {
      prompt: lang === 'no' ? `${a}    ?    ${b}\nHvilket symbol passer?` : `${a}    ?    ${b}\nWhich symbol fits?`,
      correct: op,
      choices: ['>', '<', '='],
      type: 'choice',
    };
  ...
  { id: 'sub_then_add', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 12, 22);
    const b = ri(rng, 3, 7);
    const c = ri(rng, 2, 6);
    const ans = a - b + c;
    return {
      prompt: lang === 'no' ? `${a} − ${b} + ${c} = ?\n(Gjør det fra venstre til høyre.)` : `${a} − ${b} + ${c} = ?\n(Do it left to right.)`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]),
      type: 'numeric',
    };
  } },
  { id: 'add_then_sub', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 8, 18);
    const b = ri(rng, 3, 7);
    const c = ri(rng, 2, 6);
    const ans = a + b - c;
  ...
  // ─── Ordering ──────────────────────────────────────────────────────────────
  { id: 'smallest_of_three_close', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const base = ri(rng, 15, 60);
    const a = base, b = base + ri(rng, 1, 3), c = base + ri(rng, 4, 7);
    return {
      prompt: lang === 'no' ? `Hvilken er MINST?\n${b}    ${c}    ${a}` : `Which is the SMALLEST?\n${b}    ${c}    ${a}`,
      correct: String(a),
      choices: shuffle(rng, [String(a), String(b), String(c)]),
      type: 'choice',
    };
  } },
  { id: 'biggest_of_three_close', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const base = ri(rng, 15, 60);
    const a = base, b = base + ri(rng, 1, 3), c = base + ri(rng, 4, 7);
    return {
      prompt: lang === 'no' ? `Hvilken er STØRST?\n${a}    ${b}    ${c}` : `Which is the BIGGEST?\n${a}    ${b}    ${c}`,
      correct: String(c),
      choices: shuffle(rng, [String(a), String(b), String(c)]),
      type: 'choice',
    };
  ...
  { id: 'kr_diff', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = pick(rng, ITEMS);
    ...
    return {
      prompt: lang === 'no'
      ? `${iN(hi,lang)} ${hi.emoji} koster ${hi.price} kr.\n${iN(lo,lang)} ${lo.emoji} koster ${lo.price} kr.\nHvor mye MER koster ${iN(hi,lang)}?`
      : `${iN(hi,lang)} ${hi.emoji} costs ${hi.price} kr.\n${iN(lo,lang)} ${lo.emoji} costs ${lo.price} kr.\nHow much MORE does the ${iN(hi,lang)} cost?`,
      correct: ans,
      choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2, [0, 10])]),
      type: 'numeric',
    };
  ...
  { id: '2d_add_no_carry', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    let a, b;
    do { a = ri(rng, 22, 50); b = ri(rng, 14, 25); } while (((a % 10) + (b % 10)) >= 10);
    const ans = a + b;
    return { prompt: `${a} + ${b} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: '2d_sub_no_borrow', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    let a, b;
    do { a = ri(rng, 35, 78); b = ri(rng, 12, 24); } while ((a % 10) < (b % 10));
    const ans = a - b;
    return { prompt: `${a} − ${b} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: '2d_add_easy', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 11, 28); const b = ri(rng, 5, 10);
    const ans = a + b;
    return { prompt: `${a} + ${b} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: '2d_sub_easy', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 18, 35); const b = ri(rng, 4, 10);
    const ans = a - b;
    return { prompt: `${a} − ${b} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: '2d_add_double_carry', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    let a, b;
  ...
  { id: '2d_sub_double_borrow', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    let a, b;
  ...
  { id: 'add_to_50', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 12, 32); const b = ri(rng, 8, 18);
    const ans = a + b;
    return { prompt: `${a} + ${b} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'add_to_60', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 24, 38); const b = ri(rng, 14, 22);
    const ans = a + b;
    return { prompt: `${a} + ${b} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'sub_to_30', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 38, 55); const b = ri(rng, 8, 20);
    const ans = a - b;
    return { prompt: `${a} − ${b} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'sub_to_40', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 52, 72); const b = ri(rng, 14, 24);
    const ans = a - b;
    return { prompt: `${a} − ${b} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'mid_2d_add', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 24, 46); const b = ri(rng, 18, 32);
    const ans = a + b;
    return { prompt: `${a} + ${b} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'mid_2d_sub', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 46, 74); const b = ri(rng, 18, 32);
    const ans = a - b;
    return { prompt: `${a} − ${b} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'add_two_then_one', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 18, 36); const b = ri(rng, 12, 24); const c = ri(rng, 3, 8);
    const ans = a + b + c;
    return { prompt: `${a} + ${b} + ${c} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'sub_then_one', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 38, 70); const b = ri(rng, 12, 22); const c = ri(rng, 3, 8);
    const ans = a - b - c;
    return { prompt: `${a} − ${b} − ${c} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'tens_plus_tens', difficulty: 2, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'kr_change_from_30', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = pick(rng, ITEMS); const b = pick(rng, ITEMS.filter(i => i.id !== a.id));
    const total = a.price + b.price;
    const ans = 30 - total;
    return { prompt: lang === 'no'
      ? `Vogn: ${iN(a,lang)} ${a.emoji} + ${iN(b,lang)} ${b.emoji} = ${total} kr.\nDu betaler 30 kr. Tilbake?`
      : `Cart: ${iN(a,lang)} ${a.emoji} + ${iN(b,lang)} ${b.emoji} = ${total} kr.\nYou pay 30 kr. Change?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2, [0, 30])]), type: 'numeric' };
  } },
  { id: 'kr_change_from_40', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const [a, b, c] = pickN(rng, ITEMS, 3);
    const total = a.price + b.price + c.price;
    const have = Math.max(40, total + 1);
    const ans = have - total;
    return { prompt: lang === 'no'
      ? `Total handlevogn: ${total} kr.\nDu betaler ${have} kr. Tilbake?`
      : `Cart total: ${total} kr.\nYou pay ${have} kr. Change?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'kr_change_from_100', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 18, 48); const b = ri(rng, 22, 41);
    const total = a + b;
    const ans = 100 - total;
    return { prompt: lang === 'no'
      ? `Du kjøper to ting: ${a} kr og ${b} kr.\nDu betaler med 100 kr. Tilbake?`
      : `You buy two things: ${a} kr and ${b} kr.\nYou pay with 100 kr. Change?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2, [0, 100])]), type: 'numeric' };
  } },
  { id: 'kr_can_buy_3_for_n', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const [a, b, c] = pickN(rng, ITEMS, 3);
    const total = a.price + b.price + c.price;
    const have = money ?? 15;
    ...
    return {
      prompt: lang === 'no'
      ? `Du har ${have} kr.\nHar du råd til ${iN(a,lang)} ${a.emoji} + ${iN(b,lang)} ${b.emoji} + ${iN(c,lang)} ${c.emoji}?\n(De koster ${total} kr til sammen.)`
      : `You have ${have} kr.\nCan you afford ${iN(a,lang)} ${a.emoji} + ${iN(b,lang)} ${b.emoji} + ${iN(c,lang)} ${c.emoji}?\n(They cost ${total} kr together.)`,
      correct: ans,
      choices: ['yes', 'no'],
      type: 'yesno',
    };
  } },
  { id: 'kr_save_for_target', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const item = pick(rng, ITEMS);
    const target = item.price * 5; // arbitrary save target
    const have = ri(rng, 5, target - 1);
    const ans = target - have;
    return { prompt: lang === 'no'
      ? `Du vil spare ${target} kr til et stort leketøy.\nDu har ${have} kr.\nHvor mye mer trenger du?`
      : `You want to save ${target} kr for a big toy.\nYou have ${have} kr.\nHow much more do you need?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'kr_split_three_friends', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const each = ri(rng, 4, 14);
    const total = each * 3;
    return { prompt: lang === 'no'
      ? `Tre venner deler en kostnad på ${total} kr likt.\nHvor mye betaler hver?`
      : `Three friends share a cost of ${total} kr equally.\nHow much does each one pay?`, correct: each, choices: shuffle(rng, [each, ...distinctDistractors(each, 2)]), type: 'numeric' };
  } },
  { id: 'kr_double_price', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const item = pick(rng, ITEMS);
    const ans = item.price * 2;
    return { prompt: lang === 'no'
      ? `I dag koster ${iN(item,lang)} ${item.emoji} (${item.price} kr) det DOBBELTE.\nHva er den nye prisen?`
      : `Today the ${iN(item,lang)} ${item.emoji} (${item.price} kr) costs DOUBLE.\nWhat's the new price?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'kr_half_price', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const ans = ri(rng, 3, 14);
    const v = ans * 2;
    return { prompt: lang === 'no'
      ? `Halvprissalg!\nEt leketøy som kostet ${v} kr er nå halv pris.\nHva er den nye prisen?`
      : `Half-price sale!\nA toy that was ${v} kr is now half price.\nWhat's the new price?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'kr_pay_with_coins', difficulty: 3, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'kr_swap_change', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const item = ri(rng, 17, 43);
    const ans = 50 - item;
    return { prompt: lang === 'no'
      ? `Et leketøy koster ${item} kr.\nDu gir en 50-kroneseddel.\nHva får du tilbake?`
  ...
  { id: 'kr_total_with_discount', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const a = pick(rng, ITEMS); const b = pick(rng, ITEMS.filter(i => i.id !== a.id));
    const total = a.price + b.price;
    const ans = Math.max(0, total - 5);
    return { prompt: lang === 'no'
      ? `${iN(a,lang)} ${a.emoji} (${a.price} kr) + ${iN(b,lang)} ${b.emoji} (${b.price} kr).\n5 kr rabatt. Hvor mye betaler du?`
      : `${iN(a,lang)} ${a.emoji} (${a.price} kr) + ${iN(b,lang)} ${b.emoji} (${b.price} kr).\n5 kr off the total. How much do you pay?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'kr_compare_two_baskets', difficulty: 3, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'word_seashells', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 14, 28); const b = ri(rng, 10, 22);
    const ans = a + b;
    return { prompt: lang === 'no'
      ? `På stranden 🏖 samlet Amarah ${a} skjell om morgenen\nog ${b} om ettermiddagen.\nHvor mange totalt?`
      : `At the beach 🏖, Amarah collected ${a} seashells in the morning\nand ${b} in the afternoon.\nHow many in total?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'word_pets', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const dogs = ri(rng, 6, 14); const cats = ri(rng, 5, 12);
    const ans = dogs + cats;
    return { prompt: lang === 'no'
      ? `På dyrehjemmet er det ${dogs} hunder 🐶 og ${cats} katter 🐱.\nHvor mange dyr totalt?`
      : `At the shelter there are ${dogs} dogs 🐶 and ${cats} cats 🐱.\nHow many pets total?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'word_flowers_in_garden', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const start = ri(rng, 28, 55); const picked = ri(rng, 10, 18);
    const ans = start - picked;
    return { prompt: lang === 'no'
      ? `${start} blomster 🌷 i hagen.\nDu plukker ${picked}.\nHvor mange blomster er igjen?`
      : `${start} flowers 🌷 in the garden.\nYou pick ${picked}.\nHow many flowers are left?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'word_swimmers', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const start = ri(rng, 8, 20); const more = ri(rng, 4, 12);
    const ans = start + more;
    return { prompt: lang === 'no'
      ? `${start} barn 🏊 ved bassenget.\n${more} til kommer.\nHvor mange svømmere nå?`
      : `${start} kids 🏊 at the pool.\n${more} more arrive.\nHow many swimmers now?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'word_steps_walked', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 22, 45); const b = ri(rng, 18, 35);
    const ans = a + b;
    return { prompt: lang === 'no'
      ? `I går gikk Tejas ${a} skritt til skolen\nog ${b} skritt hjem igjen.\nHvor mange skritt totalt?`
      : `Yesterday Tejas walked ${a} steps to school\nand ${b} steps back home.\nHow many steps total?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'word_drawings', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 11, 23); const b = ri(rng, 6, 14);
    const ans = a + b;
    return { prompt: lang === 'no'
      ? `Aisha tegnet ${a} bilder 🎨 på mandag\nog ${b} på tirsdag.\nHvor mange tegninger?`
      : `Aisha drew ${a} pictures 🎨 on Monday\nand ${b} on Tuesday.\nHow many drawings?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'word_buttons', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const total = ri(rng, 35, 65); const found = ri(rng, 14, 24);
    const ans = total - found;
    return { prompt: lang === 'no'
      ? `Paati har en krukke med ${total} knapper.\nHun gir ${found} til Maya.\nHvor mange er i krukken nå?`
      : `Paati has a jar of ${total} buttons.\nShe gives ${found} to Maya.\nHow many in the jar now?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'word_legos_left', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const start = ri(rng, 40, 78); const used = ri(rng, 18, 32);
    const ans = start - used;
    return { prompt: lang === 'no'
      ? `Isma hadde ${start} legobrikker 🧱.\nHan brukte ${used} til å bygge en slott.\nHvor mange legobrikker er ubrukte?`
      : `Isma had ${start} lego pieces 🧱.\nHe used ${used} to build a castle.\nHow many lego pieces are unused?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'word_treats_for_friends', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 12, 22); const b = ri(rng, 8, 16);
    const ans = a + b;
    return { prompt: lang === 'no'
      ? `Karen bakte ${a} kjeks 🍪 og ${b} cupcakes 🧁.\nHvor mange godbiter totalt?`
      : `Karen baked ${a} cookies 🍪 and ${b} cupcakes 🧁.\nHow many treats in total?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'word_jumps_on_bed', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 14, 28); const b = ri(rng, 12, 22);
    const ans = a + b;
    return { prompt: lang === 'no'
      ? `Live hoppet ${a} ganger på sengen.\nSå ${b} til!\nHvor mange hopp?`
      : `Live did ${a} jumps on the bed.\nThen ${b} more!\nHow many jumps?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'word_marshmallows', difficulty: 2, generate(rng, money = 0, lang = 'en') {
    const start = ri(rng, 16, 30); const ate = ri(rng, 6, 12);
    const ans = start - ate;
    return { prompt: lang === 'no'
      ? `En pose har ${start} marshmallows.\nDu spiser ${ate}.\nHvor mange marshmallows er igjen?`
      : `A bag has ${start} marshmallows.\nYou eat ${ate}.\nHow many marshmallows are left?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'word_socks_in_drawer', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 12, 22); const b = ri(rng, 10, 18);
    const ans = a + b;
    return { prompt: lang === 'no'
      ? `Pradeep teller ${a} stripete sokker 🧦 og ${b} enkle sokker.\nHvor mange sokker totalt?`
      : `Pradeep counts ${a} stripey socks 🧦 and ${b} plain socks.\nHow many socks total?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'word_kites_at_park', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const start = ri(rng, 18, 32); const blewAway = ri(rng, 5, 13);
    const ans = start - blewAway;
    return { prompt: lang === 'no'
      ? `Det var ${start} drager 🪁 i parken.\n${blewAway} blåste bort i vinden.\nHvor mange drager er fortsatt i luften?`
      : `There were ${start} kites 🪁 at the park.\n${blewAway} blew away in the wind.\nHow many kites still in the sky?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'word_bouncy_balls', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const total = ri(rng, 30, 55); const lost = ri(rng, 11, 22);
    const ans = total - lost;
    return { prompt: lang === 'no'
      ? `Alya hadde en bøtte med ${total} sprettballer.\n${lost} hoppet ned trappene!\nHvor mange sprettballer er igjen?`
      : `Alya had a bucket of ${total} bouncy balls.\n${lost} bounced down the stairs!\nHow many bouncy balls are left?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },

  // ─── More missing-number / fact-family (10) ────────────────────────────────
  { id: 'missing_first_2d', difficulty: 4, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'missing_second_2d', difficulty: 4, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'missing_sub_left', difficulty: 4, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'missing_sub_right', difficulty: 4, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'pattern_minus7', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const s = ri(rng, 50, 75);
    return { prompt: `${s}, ${s - 7}, ${s - 14}, ?`, correct: s - 21, choices: shuffle(rng, [s - 21, ...distinctDistractors(s - 21, 2)]), type: 'numeric' };
  } },
  { id: 'pattern_alternating', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const s = ri(rng, 5, 15);
    // pattern: +5, −2, +5, −2 (net +3 every two steps)
    return { prompt: lang === 'no' ? `${s}, ${s + 5}, ${s + 3}, ${s + 8}, ?\n(Legg til 5, trekk fra 2, gjenta.)` : `${s}, ${s + 5}, ${s + 3}, ${s + 8}, ?\n(Add 5, then subtract 2, repeat.)`, correct: s + 6, choices: shuffle(rng, [s + 6, ...distinctDistractors(s + 6, 2)]), type: 'numeric' };
  } },
  { id: 'pattern_doubling', difficulty: 4, generate(rng, money = 0, lang = 'en') {
  ...
  // ─── More comparison (10) ──────────────────────────────────────────────────
  { id: 'compare_2digit_sums', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const a1 = ri(rng, 12, 32); const a2 = ri(rng, 8, 22);
    let b1 = ri(rng, 12, 32); let b2 = ri(rng, 8, 22);
    while (a1 + a2 === b1 + b2) b2 = ri(rng, 8, 22);
    const op = (a1 + a2) > (b1 + b2) ? '>' : '<';
    return { prompt: lang === 'no'
      ? `${a1} + ${a2}    ?    ${b1} + ${b2}\nHvilket symbol passer?`
      : `${a1} + ${a2}    ?    ${b1} + ${b2}\nWhich symbol fits?`, correct: op, choices: ['>', '<', '='], type: 'choice' };
  } },
  { id: 'compare_2digit_diffs', difficulty: 4, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'equal_sums', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    ...
    return {
      prompt: lang === 'no' ? `Hvilket par summeres til ${target}?` : `Which pair adds up to ${target}?`,
      correct: `${a} + ${b}`,
      choices: shuffle(rng, [`${a} + ${b}`, wrong1, wrong2]),
      type: 'choice',
    };
  } },
  { id: 'equal_diffs', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const target = ri(rng, 5, 22);
    const a = ri(rng, target + 3, target + 18); const b = a - target;
    const wrong1 = `${a + 2} − ${b}`;
    const wrong2 = `${a} − ${b + 2}`;
    return {
      prompt: lang === 'no' ? `Hvilken er lik ${target}?` : `Which one equals ${target}?`,
      correct: `${a} − ${b}`,
      choices: shuffle(rng, [`${a} − ${b}`, wrong1, wrong2]),
      type: 'choice',
    };
  } },
  { id: 'compare_to_target_50', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const a = 45 + ri(rng, 0, 4) * 2; // 45..53
    const b = 60 + ri(rng, 0, 4); // 60..64
    const c = 30 + ri(rng, 0, 4); // 30..34
    // Closest to 50 is `a`
    return { prompt: lang === 'no' ? `Hvilket tall er NÆRMEST 50?\n${a}    ${b}    ${c}` : `Which number is CLOSEST to 50?\n${a}    ${b}    ${c}`, correct: String(a), choices: shuffle(rng, [String(a), String(b), String(c)]), type: 'choice' };
  } },
  { id: 'pick_between', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    ...
    return { prompt: lang === 'no' ? `Hvilket tall er mellom ${lo} og ${hi}?` : `Which number is between ${lo} and ${hi}?`, correct: String(inside), choices: shuffle(rng, [String(inside), String(below), String(above)]), type: 'choice' };
  } },

  // ─── More skip counting (8) ────────────────────────────────────────────────
  { id: 'skip4', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const s = ri(rng, 4, 16);
    return { prompt: lang === 'no' ? `Tell med 4: ${s}, ${s + 4}, ${s + 8}, ?` : `Counting by 4s: ${s}, ${s + 4}, ${s + 8}, ?`, correct: s + 12, choices: shuffle(rng, [s + 12, ...distinctDistractors(s + 12, 2)]), type: 'numeric' };
  } },
  { id: 'skip6', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const s = ri(rng, 6, 18);
    return { prompt: lang === 'no' ? `Tell med 6: ${s}, ${s + 6}, ${s + 12}, ?` : `Counting by 6s: ${s}, ${s + 6}, ${s + 12}, ?`, correct: s + 18, choices: shuffle(rng, [s + 18, ...distinctDistractors(s + 18, 2)]), type: 'numeric' };
  } },
  { id: 'skip7', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const s = ri(rng, 7, 14);
    return { prompt: lang === 'no' ? `Tell med 7: ${s}, ${s + 7}, ${s + 14}, ?` : `Counting by 7s: ${s}, ${s + 7}, ${s + 14}, ?`, correct: s + 21, choices: shuffle(rng, [s + 21, ...distinctDistractors(s + 21, 2)]), type: 'numeric' };
  } },
  { id: 'skip8', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const s = ri(rng, 8, 16);
    return { prompt: lang === 'no' ? `Tell med 8: ${s}, ${s + 8}, ${s + 16}, ?` : `Counting by 8s: ${s}, ${s + 8}, ${s + 16}, ?`, correct: s + 24, choices: shuffle(rng, [s + 24, ...distinctDistractors(s + 24, 2)]), type: 'numeric' };
  } },
  { id: 'skip9', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const s = ri(rng, 9, 18);
    return { prompt: lang === 'no' ? `Tell med 9: ${s}, ${s + 9}, ${s + 18}, ?` : `Counting by 9s: ${s}, ${s + 9}, ${s + 18}, ?`, correct: s + 27, choices: shuffle(rng, [s + 27, ...distinctDistractors(s + 27, 2)]), type: 'numeric' };
  } },
  { id: 'skip_back3', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    const s = ri(rng, 30, 60);
    return { prompt: lang === 'no' ? `Tell tilbake med 3: ${s}, ${s - 3}, ${s - 6}, ?` : `Counting back by 3s: ${s}, ${s - 3}, ${s - 6}, ?`, correct: s - 9, choices: shuffle(rng, [s - 9, ...distinctDistractors(s - 9, 2)]), type: 'numeric' };
  } },
  { id: 'skip_back5', difficulty: 3, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'time_full_year_months', difficulty: 3, generate(rng, money = 0, lang = 'en') {
    return { prompt: lang === 'no' ? `Hvor mange måneder er det i et år?` : `How many months are in a year?`, correct: 12, choices: shuffle(rng, [12, 10, 14]), type: 'numeric' };
  } },
  ...
  // ─── v7 plan extras (6) ────────────────────────────────────────────────────
  { id: 'add_three_2d', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 15, 28); const b = ri(rng, 12, 22); const c = ri(rng, 8, 18);
    const ans = a + b + c;
    return { prompt: `${a} + ${b} + ${c} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'sub_chain', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const a = ri(rng, 40, 70); const b = ri(rng, 12, 22); const c = ri(rng, 5, 12);
    const ans = a - b - c;
    return { prompt: `${a} − ${b} − ${c} = ?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'find_addend', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const b = ri(rng, 18, 38); const ans = ri(rng, 14, 28);
    const total = b + ans;
    return { prompt: `? + ${b} = ${total}`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'before_and_after', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const start = ri(rng, 30, 60); const gave = ri(rng, 8, 18); const got = ri(rng, 5, 14);
    const ans = start - gave + got;
    return { prompt: lang === 'no'
      ? `Maya hadde ${start} klistremerker ✨.\nHun ga ${gave} til en venn\nog fikk ${got} til.\nHvor mange nå?`
      : `Maya had ${start} stickers ✨.\nShe gave ${gave} to a friend\nthen got ${got} more.\nHow many now?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2)]), type: 'numeric' };
  } },
  { id: 'compare_2digit_sums_v7', difficulty: 4, generate(rng, money = 0, lang = 'en') {
  ...
  { id: 'kr_three_items_change', difficulty: 4, generate(rng, money = 0, lang = 'en') {
    const [a, b, c] = pickN(rng, ITEMS, 3);
    const total = a.price + b.price + c.price;
    const ans = 50 - total;
    return { prompt: lang === 'no'
      ? `Vogn: ${iN(a,lang)} ${a.emoji} + ${iN(b,lang)} ${b.emoji} + ${iN(c,lang)} ${c.emoji} (${total} kr).\nDu betaler med 50 kr.\nTilbake?`
      : `Cart: ${iN(a,lang)} ${a.emoji} + ${iN(b,lang)} ${b.emoji} + ${iN(c,lang)} ${c.emoji} (${total} kr).\nYou pay with 50 kr.\nChange?`, correct: ans, choices: shuffle(rng, [ans, ...distinctDistractors(ans, 2, [0, 50])]), type: 'numeric' };
  } },
];
...
// ─────────────────────────────────────────────────────────────────────────────
export function pickProblems(count, money, seed = Date.now(), lang = 'en') {
  ...
  for (const tier of plan) {
    ...
    used.add(tmpl.id);
    out.push(tmpl.generate(rng, money, lang));
  }
  return out;
}
